...
const GenerateReport: React.FC = () => {
  ...
  const handleImageUpload = useCallback((files: FileList | null, type: 'before' | 'after' | 'upload') => {
    ...
    <div className="min-h-screen bg-ios-background">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 pt-24">
        <div className="bg-white/60 backdrop-blur-sm p-2 rounded-2xl mb-8 border border-gray-200/50 shadow-md">
          <div className="flex">
            <button onClick={() => triggerFileInput('before')} className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-medium transition-all duration-300 active:scale-95 ${activeTab === 'before' ? 'bg-red-500 text-white shadow-lg shadow-red-500/25' : 'text-gray-600 hover:text-black'}`}>
              <Camera className="h-4 w-4" /> Before ({beforeCount})
            </button>
            <button onClick={() => triggerFileInput('after')} className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-medium transition-all duration-300 active:scale-95 ${activeTab === 'after' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/25' : 'text-gray-600 hover:text-black'}`}>
              <Camera className="h-4 w-4" /> After ({afterCount})
            </button>
            <button onClick={() => triggerFileInput('upload')} className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-medium transition-all duration-300 active:scale-95 ${activeTab === 'upload' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/25' : 'text-gray-600 hover:text-black'}`}>
              <Upload className="h-4 w-4" /> Upload ({uploadCount})
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Button onClick={handleSaveOrUpdateDraft} className="bg-indigo-500/90 hover:bg-indigo-500 text-white border-0 px-8 py-4 rounded-2xl font-medium text-lg shadow-lg shadow-indigo-500/25 hover:shadow-xl hover:shadow-indigo-500/30 transition-all duration-300 active:scale-95 backdrop-blur-sm flex items-center gap-3">
            <Save className="h-5 w-5" /> {currentDraftId ? 'Update Draft' : 'Save as Draft'}
          </Button>
          <Button onClick={() => setShowExportOptions(true)} disabled={isGeneratingPDF} className="bg-emerald-600/90 hover:bg-emerald-600 text-white border-0 px-8 py-4 rounded-2xl font-medium text-lg shadow-lg shadow-emerald-600/25 hover:shadow-xl hover:shadow-emerald-600/30 transition-all duration-300 active:scale-95 backdrop-blur-sm flex items-center gap-3 disabled:opacity-50">
            <Download className="h-5 w-5" /> {isGeneratingPDF ? 'Generating...' : 'Export PDF'}
          </Button>
        </div>
      </main>
